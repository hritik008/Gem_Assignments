...
function validate() {
  ...
  let validMobile = /^[789]\d{9}$/.test(phoneNo.value);
  ...
  if (phoneNo.value === "" || validMobile) {
    small[2].style.visibility = "hidden";
    phoneNo.style.borderColor = "#CED4DA";
  } else {
  ...
  if (allValid) {
    insertOnTable(name.value, email.value, phoneNo.value);
  }
}

function insertOnTable(name, email, phoneNo) {
  let details = {
    name: name,
    email: email,
    phoneNo: phoneNo
  ...
  createRow(c, details.name, details.email, details.phoneNo);
}
...
function createRow(no, name, email, phoneNo) {
